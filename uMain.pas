...
type
  TfrmMain = class(TfrmForm)
    ...
    procedure ListViewMainColumnClick(Sender: TObject;
      Column: TListColumn);
    procedure ListViewMainCompare(Sender: TObject; Item1, Item2: TListItem;
...
begin
  ...
  for var I := 0 to ListViewMain.Items.Count - 1 do if ListViewMain.Items[I].Selected and Assigned(ListViewMain.Items[I].Data) then TARObject(ListViewMain.Items[I].Data).Active := True;
end;
...
begin
  inherited;
  for var I := 0 to ListViewMain.Items.Count - 1 do if ListViewMain.Items[I].Selected and Assigned(ListViewMain.Items[I].Data) then TARObject(ListViewMain.Items[I].Data).Active := False;
end;
...
begin
  inherited;
  for var I := 0 to ListViewMain.Items.Count - 1 do if ListViewMain.Items[I].Selected then TARObject(ListViewMain.Items[I].Data).Modified := not TARObject(ListViewMain.Items[I].Data).Modified;
end;

function TfrmMain.GetGroupIndex(GroupID: Integer): Integer;
begin
  Result := -1;
  for var I := 0 to ListViewMain.Groups.Count - 1 do if ListViewMain.Groups[I].GroupID = GroupID then begin
...
begin
  Result := -1;
  for var I := 0 to ListViewMain.Groups.Count - 1 do if lgsFocused in ListViewMain.Groups[I].State then begin
    Result := ListViewMain.Groups[I].Index;
    Break;
  end;
end;
...
begin
  Result := 0;
  for var I := 0 to ListViewMain.Items.Count - 1 do if ListViewMain.Items[I].Checked then Inc(Result);
end;

function TfrmMain.GetFirstGroupItem(GroupID: Integer): TListItem;
begin
  Result := nil;
  for var I := 0 to ListViewMain.Items.Count - 1 do if ListViewMain.Items[I].GroupID = GroupID then begin
...
begin
  ...
  for var I := 0 to iNode.ChildNodes.Count - 1 do if iNode.ChildNodes[I].Attributes[ND_PARAM_ID] = IntToStr(GroupID) then begin
...
function TfrmMain.GetNewGroupName: String;
   ...
   begin
     Result := False;
     for var I := 0 to ListViewMain.Groups.Count - 1 do begin
...
begin
  ...
  ListViewMain.Items.BeginUpdate;
  try
    for var I := 0 to ListViewMain.Groups.Count - 1 do ListViewMain.Groups[I].State := ListViewMain.Groups[I].State + [lgsCollapsed]
  finally
    ListViewMain.Items.EndUpdate;
  end;
end;
...
begin
  inherited;
  if ListViewMain.SelCount = 1 then TARObject(ListViewMain.Selected.Data).Run else for var I := 0 to ListViewMain.Items.Count - 1 do if ListViewMain.Items[I].Selected then TARObject(ListViewMain.Items[I].Data).Run;
end;
...
begin
  ...
  if MessageBox(Handle, PChar(ICSLanguagesMessages.CurrentStrings[APP_ARSERVICE_MESSAGE_ID_DELETE_HOSTS]), PChar(Application.Title), MB_YESNO + MB_ICONQUESTION + MB_DEFBUTTON2) = ID_YES then begin
    for var I := 0 to ListViewMain.Items.Count - 1 do if ListViewMain.Items[I].Selected then DeleteListItemData(ListViewMain.Items[I]);
...
begin
  ...
  for var I := (Sender as TMenuItem).Count - 1 downto 0 do if (Sender as TMenuItem).Items[I].HelpContext = 1 then (Sender as TMenuItem).Delete(I);
  for var I := 0 to ListViewMain.Groups.Count - 1 do if ListViewMain.Groups[I].GroupID <> APP_ARGROUP_ID_FAVORITES then begin
...
begin
  ...
  if MessageBox(Handle, PChar(ICSLanguagesMessages.CurrentStrings[APP_ARSERVICE_MESSAGE_ID_DELETE_GROUPS]), PChar(Application.Title), MB_YESNO + MB_ICONQUESTION + MB_DEFBUTTON2) = ID_YES then begin
    ListViewMain.Items.BeginUpdate;
    try
      for var I := ListViewMain.Groups.Count - 1 downto 0 do if ListViewMain.Groups[I].GroupID < APP_ARGROUP_ID_FIRSTRESERVED then begin
        ItemsFound := False;
        for var J := 0 to ListViewMain.Items.Count - 1 do if ListViewMain.Groups[I].GroupID = ListViewMain.Items[J].GroupID then begin
          ItemsFound := True;
          Break;
        end;
...
begin
  for var I := 0 to ListViewMain.Items.Count - 1 do if ListViewMain.Items[I].Selected then begin
  ...
  if ListViewMain.SelCount = 1 then begin
    ...
    try
      ...
      if EditHostNode(iNode, True) then begin
        for var I := 0 to ListViewMain.Items.Count - 1 do if ListViewMain.Items[I].Selected then begin
          for var J := 0 to iNode.ChildNodes.Count - 1 do begin
...
begin
  ...
  SaveGroupProperties;
  SaveBounds;
  if Assigned(ListViewMain.ItemFocused) then xmlSetItemString(FXML.DocumentElement.ChildNodes[ND_HOSTS], ND_PARAM_FOCUSED, xmlGetItemString(TARObject(ListViewMain.ItemFocused.Data).XMLNode, ND_PARAM_ID));
...
begin
  inherited;
  if ColumnToSort = 0 then Compare := CompareText(Item1.Caption,Item2.Caption) else begin
    ix := ColumnToSort - 1;
...
procedure TfrmMain.NormalizeGroups;
 var argGeneralExists, argFavoritesExists: Boolean;
begin
...
begin
  ...
  try
    ...
    iNode := FXML.DocumentElement.ChildNodes[ND_GROUPS];
    for var I := 0 to iNode.ChildNodes.Count - 1 do CreateGroupItem(iNode.ChildNodes[I]);
    NormalizeGroups;
    for var I := 0 to ListViewMain.Items.Count - 1 do if Assigned(ListViewMain.Items[I].Data) then TARObject(ListViewMain.Items[I].Data).TerminateCheckStateThread;
    ListViewMain.Clear;
    iNode := FXML.DocumentElement.ChildNodes[ND_HOSTS];
    for var I := 0 to iNode.ChildNodes.Count - 1 do CreateHostItem(iNode.ChildNodes[I]);
    for var I := 0 to ListViewMain.Items.Count - 1 do if xmlGetItemString(FXML.DocumentElement.ChildNodes[ND_HOSTS], ND_PARAM_FOCUSED) = xmlGetItemString(TARObject(ListViewMain.Items[I].Data).XMLNode, ND_PARAM_ID) then begin
...
begin
  for var I := 0 to ListViewMain.Items.Count -1 do ApplyHostParams(ListViewMain.Items[I]);
end;

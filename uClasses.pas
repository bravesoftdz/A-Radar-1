...
procedure TARObject.BuildARDataRec(var ARObjectData: TARObjectData);
  ...
  begin
    for var I := SL.Count - 1 downto 0 do if not SL[I].IsEmpty and CharInSet(SL[I].Chars[1], [';', '#']) then SL.Delete(I);
  end;
  ...
  begin
    ...
    try
      ...
      try
        for var I := 0 to SLParams.Count - 1 do if sNameValueSeparator <> #0 then begin
...
begin
  ...
  for var I := 0 to iNode.ChildNodes.Count - 1 do if xmlGetItemString(iNode.ChildNodes[I], ND_PARAM_ID) = SoftId then begin
...
begin
  ...
  for var I := 0 to iNode.ChildNodes.Count - 1 do if xmlGetItemString(iNode.ChildNodes[I], ND_PARAM_ID) = ProtocolId then begin
    Result := iNode.ChildNodes[I];
    Break;
  end;
end;
...
begin
  ...
  for var I := 0 to iNode.ChildNodes.Count - 1 do if xmlGetItemString(iNode.ChildNodes[I], ND_PARAM_ID) = sId then begin
    Result := iNode.ChildNodes[I];
    Break;
  end;
end;
...
//              MessageBox(0, PChar(AROD.CMDParams), PChar(AROD.CMD), MB_OK); Exit;
        ...
        if bExecuteResult and AROD.UnsetAfterOpen then FOwner.Modified := False;
...
begin
  for var I := 0 to Count - 1 do ARObjects[I].Free;
  inherited Clear;
end;
...
begin
  for var I := 0 to Count - 1 do if ARObjects[I] = Item then begin
    Delete(I);
    Break;
  end;
end;
